...
Why this exists
----------------
"It seems to work when I try it" isn't a metric. RAGAS scores each
question against four dimensions:

  - faithfulness        : is the answer actually supported by the
                           retrieved context, or did the LLM add things
                           that aren't there (hallucination)?
  - answer_relevancy     : does the answer actually address the question
                           asked (not just sound plausible)?
  - context_precision    : of the chunks retrieved, how many were
                           actually relevant? (retrieval quality)
  - context_recall       : of what the reference answer needed, how much
                           did retrieval actually surface? (needs a
                           ground_truth reference answer per question)

Run this BEFORE and AFTER a retrieval change (e.g. switching from plain
vector search to the hybrid retriever) to get a real before/after number
instead of a vibe.
...
This uses a local Ollama model as the judge, so evaluation stays fully
local like the rest of the app. Judge quality with a small local model is
noticeably weaker than GPT-4-class judges — treat scores as directionally
useful for comparing two versions of YOUR pipeline, not as an absolute
benchmark against other systems.

A NOTE ON RELIABILITY: faithfulness and context_precision ask the judge
LLM to return strict, structured JSON verdicts (one per claim / one per
retrieved chunk). Small local 7B models are unreliable at producing
valid JSON on the first try, which triggers internal retries — each one
a full slow local generation — and often exhausts the timeout before a
parseable answer ever comes back. answer_relevancy and context_recall
ask for looser output and tend to succeed much more reliably as a
result. If you see persistent NaNs on faithfulness/context_precision
after trying a better JUDGE_MODEL below, that's a known limitation of
small local judges, not a bug in this script — worth calling out
directly if this comes up in an interview.

Usage
-----
1. Make sure you've already processed at least one PDF through app.py
   (so ./chroma_db exists).
2. Fill in TEST_SET below with real questions about YOUR uploaded PDF(s)
   and the correct reference answer for each.
3. (Recommended) Pull a more instruction-tuned model for judging —
   these tend to produce valid JSON far more reliably than mistral:
     ollama pull qwen2.5:7b-instruct
   or, if you have the RAM/patience for something bigger:
     ollama pull llama3.1:8b-instruct
   Then set JUDGE_MODEL below to match.
4. Run: python eval_ragas.py
"""
